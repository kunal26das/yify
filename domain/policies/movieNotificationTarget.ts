export interface MovieNotificationTarget {
    movieId?: number;
    count?: number;
    kind: 'daily-pick' | 'new-release' | 'availability';
}

export function movieNotificationTarget(data: unknown): MovieNotificationTarget | null {
    if (data == null || typeof data !== 'object' || Array.isArray(data)) return null;
    const value = data as Record<string, unknown>;
    const kind = value.kind === 'daily-pick' ? 'daily-pick' : value.kind === 'availability' ? 'availability' : 'new-release';
    ...
    if ((kind === 'new-release' || kind === 'availability') && typeof value.count === 'number' && Number.isSafeInteger(value.count) && value.count > 0) {
