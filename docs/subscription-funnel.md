...
The checkout and prompt event names are retained from the existing implementation. Their `funnel_version: "v1"` payload replaces package identifiers with bounded categories. RevenueCat custom paywall impressions continue independently with their existing offering context. Repeated checkout taps share one attempt; refreshed plans do not create another offers-visible event in the same account's open dialog. A different account starts a fresh exposure.

Categorical values use strings on every platform: `funnel_version` is `"v1"`, `saved_milestone` is `"one"` or `"three"`, and `signed_in`, `supporter_access`, `granted` and `enabled` are `"true"` or `"false"`. `offer_count` remains a numeric count. Google documents that integer app event parameters are not parsed as event-scoped custom dimensions, and Firebase Android supports string, long and double event values. The prior numeric version/milestone and boolean access payloads could therefore be missing from app dimension reports. The updated encoding does not restore that history or alter any billing, revenue or entitlement behavior. See [GA4 custom dimension types](https://support.google.com/analytics/answer/14239696?hl=en) and [Firebase Android event parameter types](https://firebase.google.com/docs/reference/android/com/google/firebase/analytics/FirebaseAnalytics#logEvent(java.lang.String,android.os.Bundle)).
...
On September 23, 2026, the nine missing definitions were created in property `292918173`. The existing `reason` definition was retained under its display name **Ad gate reason**; use that definition for Checkout outcome reason rather than creating a duplicate. New definitions do not recover past parameter history. Allow 24–48 hours after the parameters are sent and definitions are created, then validate actual rows before starting the measured experiment window. The new string encoding also needs to reach clients before it can establish a complete app baseline.

Use DebugView with a debug device to confirm the string values above; do not generate production purchases for validation. Allow 24–48 hours after custom data is sent and definitions are created for report availability before treating missing rows as a tracking defect. Definitions do not reconstruct previously unavailable custom-dimension history.

Create separate explorations:

1. **Funnel activity:** Free form; rows Event name and Date; columns Platform; values Event count and Total users. Filter event names to the table above and Funnel version to `v1`; include legacy `1` only when reviewing a mixed-release window and disclose the missing app-history limitation. Add Viewing country and Paywall placement as filters. Keep event count and users separate; do not total users across rows.
2. **Checkout journey:** Closed funnel; step 1 `supporter_offers_visible`, step 2 `remove_ads_purchase_start`, step 3 `remove_ads_purchase_done` with Checkout access granted equal to `true` (a string). Require indirect succession within seven days, filter Funnel version to `v1`, and break down by Platform or Paywall placement. For legacy comparisons, also include verified legacy `1` values for version/access where available. This measures ordered analytics users receiving access, not paid subscriptions. It is not an acquisition-cohort purchase-conversion report.
...
4. **Journal-to-offer journey:** Use user-level ordered steps: `journal_action`/`entry_created`, `journal_action`/`insights_opened`, then `supporter_offers_visible`, `remove_ads_purchase_start`, and `remove_ads_purchase_done` with granted access. Limit the checkout steps to `placement: journal_insights` and `funnel_version: "v1"`; never apply that version filter to the journal steps. Use a defined seven-day completion window and distinguish existing supporters from the non-supporter cohort. This records an observed analytics journey, not a causal effect or a paid transaction.

For activation, use a separate user segment with `watchlist_activation` and Saved title milestone `three` (also legacy `3` when comparing available older data); compare date-bounded acquisition cohorts rather than dividing independent event totals. Use RevenueCat production cohorts separately for first payment, renewal and subscriber retention. Do not mark custom checkout outcomes as additional revenue events.
