...
export class AuthView {
    ...
    constructor(vm: AuthViewModel, logVm: LogViewModel) {
        ...
        $('#accessToken').addEventListener('input', () => this.#syncKeyAffix());
        $('#accessToken').addEventListener('keydown', (e: KeyboardEvent) => {
            if (e.key === 'Enter') this.#login();
        });
    }

    #render({info, loggedIn, name}: AuthChangeState): void {
    ...
    async #copyCmd(): Promise<void> {
        ...
        try {
            await navigator.clipboard.writeText('eas login');
    ...
    #syncKeyAffix(): void {
        ...
        btn.setAttribute(
            'aria-label',
            hasText ? 'Clear access token' : 'Paste access token',
        );
    ...
    async #openLogin(): Promise<void> {
        ...
        try {
            ...
            gs.textContent =
                'Opened the login page in your browser — copy your access token, then paste it above.';
    ...
    async #login(): Promise<void> {
        const gs = $('#gateStatus');
        const keyInput = $<HTMLInputElement>('#accessToken');
        const key = keyInput.value.trim();
        if (!key) {
            gs.textContent = 'Paste your access token first.';
            gs.className = 'status warn';
            showToast('Paste your access token first.', 'warn');
            keyInput.focus();
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                gs.className = 'status ok';
                showToast('Signed in to Expo.');
                await this.vm.refresh();
            } else {
                gs.textContent =
                    'Login failed — double-check the access token and try again.';
                gs.className = 'status bad';
                showToast('Login failed — check the access token.', 'bad');
            }
        } catch (e) {
            gs.textContent = errMessage(e);
            gs.className = 'status bad';
