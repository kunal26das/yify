...
const FIREBASE_GROUP = process.env.YIFY_RELEASE_FIREBASE_GROUP || 'yify';
...
function serviceAccountPath(): string {
    return (
        process.env.YIFY_RELEASE_PLAY_SERVICE_ACCOUNT ||
...
function firebaseServiceAccountPath(): string {
    return (
        process.env.YIFY_RELEASE_FIREBASE_SERVICE_ACCOUNT ||
...
export function createAndroidPublisher(deps: {
    ...
    function releaseNotes(): string {
        const v = appVersion();
        return v
            ? `Yify Staging v${v} — store release`
            : 'Yify Staging — store release';
    }

    function firebaseAppRef(): { appId: string; projectNumber: string } {
        const envAppId = process.env.YIFY_RELEASE_FIREBASE_APP_ID;
        const envNumber = process.env.YIFY_RELEASE_FIREBASE_PROJECT_NUMBER;
        ...
        if (!gsPath) {
            throw new Error(
                'google-services.json not found (looked in android/app and config/android). Set YIFY_RELEASE_FIREBASE_APP_ID and YIFY_RELEASE_FIREBASE_PROJECT_NUMBER.',
            );
        }
    ...
    async function publishProduction(
        ...
        try {
            ...
            if (!fs.existsSync(saFile)) {
                onLine({
                    stream: 'stderr',
                    text: `Play service account not found at ${saFile}. Set YIFY_RELEASE_PLAY_SERVICE_ACCOUNT or place the JSON key there.`,
                    label,
                });
                return {ok: false};
            }
